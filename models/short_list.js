...
module.exports.getGeneGains = (exspecies, anspecies,page, limit, callback) => {
    ShortGeneList.find({$and: [{$not: {'ancestor_species': new RegExp(anspecies), 'pthr': 'NOT_AVAILABLE'}}, {$or: [{'species': exspecies}, {'species_long': exspecies}]}]}, {'_id':0,'species':0}).skip(limit*(page-1)).limit(limit).exec(callback);
}
