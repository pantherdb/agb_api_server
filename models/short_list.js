...
module.exports.getGeneGains = (exspecies, anspecies,page, limit, callback) => {
    ShortGeneList.find({$and: [{$not: {'ancestor_species': new RegExp(anspecies)}}, {$or: [{'species': exspecies}, {'species_long': exspecies}]}, {$not: {'pthr': 'NOT_AVAILABLE'}}]}, {'_id':0,'species':0}).skip(limit*(page-1)).limit(limit).exec(callback);
}
